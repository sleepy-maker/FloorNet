        ...
        for line in f:
            ...
            if eles[0] == "v":
                if texture_exists:
                    ...
                    pnt = [float(eles[1]), float(eles[2]), float(eles[3]), float(eles[4]), float(eles[5]),
                           float(eles[6])]
                    pass
                all_pnts.append(pnt)
...
class PntMapper:
    ...
    def map_pnts(self, x, y):
        print(self.size)
        return (x - self.start[0]) / (self.size[0] - 1) * (self.newsize[0] - 1) + self.newstart[0], (
                    y - self.start[1]) / (self.size[1] - 1) * (self.newsize[1] - 1) + self.newstart[1]
...
class RecordWriterTango():
    ...
    def loadLabelMap(self):
        ...
        labelMap = {}
        for icon in ['cooking_counter', 'bathtub', 'toilet', 'washing_basin', 'sofa', 'cabinet', 'bed', 'table', 'desk',
                     'refrigerator', 'TV', 'entrance', 'chair']:
            if icon not in iconMap:
                print(icon)
                exit(1)
            labelMap[icon] = ('icons', iconMap[icon])
            continue
        for room in ['living_room', 'kitchen', 'bedroom', 'bathroom', 'office', 'closet', 'balcony', 'corridor',
                     'dining_room', 'stairs']:
    ...
    def writeExample(self, scene_id):
        ...
        XYZ = np.tensordot(np.concatenate([points[:, :3], np.ones((points.shape[0], 1))], axis=1), globalTransformation,
                           axes=((1), (1)))
        XYZ = XYZ[:, :3] / XYZ[:, 3:]
        ...
        self.projectIndices(np.concatenate([coordinates, np.arange(NUM_POINTS).reshape(-1, 1)], axis=1), 0, WIDTH, 0,
                            HEIGHT)
        ...
        with open(filename) as info_file:
            ...
            for line in info_file:
                line = line.split('\t')
                if line[4] == 'wall':
                    walls.append(((transformPoint(line[0], 0), transformPoint(line[1], 1)),
                                  (transformPoint(line[2], 0), transformPoint(line[3], 1))))
                elif line[4] == 'door':
                    doors.append(((transformPoint(line[0], 0), transformPoint(line[1], 1)),
                                  (transformPoint(line[2], 0), transformPoint(line[3], 1))))
                elif line[4] == 'window':
                    windows.append(((transformPoint(line[0], 0), transformPoint(line[1], 1)),
                                    (transformPoint(line[2], 0), transformPoint(line[3], 1))))
                else:
                    if line[4] not in semantics:
                        semantics[line[4]] = []
                        pass
                    semantics[line[4]].append(((transformPoint(line[0], 0), transformPoint(line[1], 1)),
                                               (transformPoint(line[2], 0), transformPoint(line[3], 1))))
                    pass
                continue
            pass
        ...
        for line in walls:
            cv2.line(roomSegmentation, (int(round(line[0][0])), int(round(line[0][1]))),
                     (int(round(line[1][0])), int(round(line[1][1]))), color=15 + calcLineDirection(line),
                     thickness=self.gap)
        ...
        for line in doors:
            cv2.line(iconSegmentation, (int(round(line[0][0])), int(round(line[0][1]))),
                     (int(round(line[1][0])), int(round(line[1][1]))), color=self.labelMap['door'],
                     thickness=self.gap - 1)
            continue
        for line in windows:
            cv2.line(iconSegmentation, (int(round(line[0][0])), int(round(line[0][1]))),
                     (int(round(line[1][0])), int(round(line[1][1]))), color=self.labelMap['window'],
                     thickness=self.gap - 1)
            continue
        ...
        for semantic, items in semantics.iteritems():
            ...
            for corners in items:
                if group == 'icons':
                    if label == 0:
                        continue
                    cv2.rectangle(iconSegmentation, (int(round(corners[0][0])), int(round(corners[0][1]))),
                    ...
                    roomIndex = rooms[int(round((corners[0][1] + corners[1][1]) / 2))][
                        int(round((corners[0][0] + corners[1][0]) / 2))]
        ...
        if os.path.exists(corner_feature_file) and os.path.exists(icon_feature_file):
            ...
            for featureIndex, feature in enumerate([corner_feature, icon_feature]):
                for size, numChannels in zip(SIZES, NUM_CHANNELS)[1:]:
                    ...
                    image_features[featureIndex].append(
                        feature.reshape((size, size, numChannels, -1)).mean(-1).reshape(-1))
                    continue
            ...
            image_features = np.zeros(
                sum([size * size * numChannels for size, numChannels in zip(SIZES, NUM_CHANNELS)[1:]]))
            pass

        if False:
            cv2.imwrite('test/density.png', drawDensityImage(getDensity(points, HEIGHT, WIDTH)))
            cv2.imwrite('test/density_indices.png',
                        drawDensityImage(getDensityFromIndices(self.indicesMaps, HEIGHT, WIDTH)))
