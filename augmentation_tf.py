...
def getCoarseIndicesMaps(indicesMap, width=256, height=256, batchIndex=0):
    ...
    for strideIndex in xrange(6):
        ...
        if strideIndex == 0:
            ...
            indicesMaps.append(indicesMap / (width * stride) * (
                        width / stride) + indicesMap % width / stride + batchIndex * width / stride * height / stride)
            pass
        # print(indicesMaps)
        continue
...
def augmentWarping(pointcloudIndices, corners, heatmaps, gridStride=16, randomScale=4):
    ...
    xsTarget, ysTarget = warpIndices(pointcloudIndices % width, pointcloudIndices / width, gridStride, gridWidth,
                                     gridHeight, width, height, gridXsTarget, gridYsTarget)

    newPointcloudIndices = tf.clip_by_value(ysTarget, 0, height - 1) * width + tf.clip_by_value(xsTarget, 0, width - 1)

    xsTarget, ysTarget = warpIndices(corners[:, 0], corners[:, 1], gridStride, gridWidth, gridHeight, width, height,
                                     gridXsTarget, gridYsTarget)
...
def augmentScaling(pointcloud, pointcloudIndices, corners, heatmaps, imageFeatures):
    ...
    xsTarget = tf.clip_by_value(
        tf.cast(tf.round((tf.cast(xs, tf.float32) - width / 2) * randomScale + width / 2), tf.int32), 0, width - 1)
    ysTarget = tf.clip_by_value(
        tf.cast(tf.round((tf.cast(ys, tf.float32) - height / 2) * randomScale + height / 2), tf.int32), 0, height - 1)
    ...
    newCorners = tf.stack([xsTarget, ysTarget, corners[:, 2]], axis=1)
    validMask = tf.logical_and(
        tf.logical_and(tf.greater_equal(newCorners[:, 0], 0), tf.greater_equal(newCorners[:, 1], 0)),
        tf.logical_and(tf.less(newCorners[:, 0], WIDTH), tf.less(newCorners[:, 1], HEIGHT)))
    ...
    for index, (featureSize, numChannels) in enumerate(zip(SIZES, NUM_CHANNELS)[1:]):
        if index in imageFeatures:
            imageFeatures[index] = tf.image.resize_image_with_crop_or_pad(
                tf.image.resize_nearest_neighbor(tf.expand_dims(imageFeatures[index], 0), size=tf.cast(
                    tf.round(tf.constant((featureSize, featureSize), dtype=tf.float32) * randomScale), tf.int32)),
                featureSize, featureSize)[0]
            pass
        continue
...
def augmentFlipping(pointcloud, pointcloudIndices, corners, heatmaps, imageFeatures):
    ...
    reverseChannelsY = tf.constant([-1, 2, 1, 0, 3, 7, 6, 5, 4, 10, 9, 8, 11, 12, 15, 14, 13, 16, 20, 19, 18, 17],
                                   dtype=tf.int32) + 1
    reverseChannelsX = tf.constant([-1, 0, 3, 2, 1, 5, 4, 7, 6, 8, 11, 10, 9, 12, 13, 16, 15, 14, 18, 17, 20, 19],
                                   dtype=tf.int32) + 1

    xsTarget = tf.cond(orientation >= 2, lambda: width - 1 - xsTarget, lambda: xsTarget)
    pointcloud = tf.cond(orientation >= 2, lambda: tf.concat([1 - pointcloud[:, 0:1], pointcloud[:, 1:]], axis=1),
                         lambda: pointcloud)
    heatmaps = tf.cond(orientation >= 2, lambda: heatmaps[:, :, ::-1], lambda: heatmaps)
    corners = tf.cond(orientation >= 2, lambda: tf.stack(
        [width - 1 - corners[:, 0], corners[:, 1], tf.gather_nd(reverseChannelsX, corners[:, 2:3])], axis=1),
                      lambda: corners)

    ysTarget = tf.cond(tf.equal(orientation % 2, 1), lambda: height - 1 - ysTarget, lambda: ysTarget)
    pointcloud = tf.cond(tf.equal(orientation % 2, 1),
                         lambda: tf.concat([pointcloud[:, :1], 1 - pointcloud[:, 1:2], pointcloud[:, 2:]], axis=1),
                         lambda: pointcloud)
    heatmaps = tf.cond(tf.equal(orientation % 2, 1), lambda: heatmaps[:, ::-1], lambda: heatmaps)
    corners = tf.cond(tf.equal(orientation % 2, 1), lambda: tf.stack(
        [corners[:, 0], height - 1 - corners[:, 1], tf.gather_nd(reverseChannelsY, corners[:, 2:3])], axis=1),
                      lambda: corners)

    for index, (size, numChannels) in enumerate(zip(SIZES, NUM_CHANNELS)[1:]):
        if index in imageFeatures:
            imageFeatures[index] = tf.cond(orientation >= 2, lambda: imageFeatures[index][:, ::-1],
                                           lambda: imageFeatures[index])
            imageFeatures[index] = tf.cond(tf.equal(orientation % 2, 1), lambda: imageFeatures[index][::-1],
                                           lambda: imageFeatures[index])
            pass
        continue
